...
break;case 10:d+="November";break;case 11:d+="December"}break;case "m":var h=""+(c.getMonth()+1);1==h.length&&(h="0"+h);d+=h;break;case "Y":d+=c.getFullYear();break;case "G":var k=""+c.getHours(),d=d+k;break;case "H":k=""+c.getHours();1==k.length&&(k="0"+k);d+=k;break;case "i":var l=""+c.getMinutes();1==l.length&&(l="0"+l);d+=l;break;default:d+=f}}return d}catch(m){console.log("date - "+m)}}
...
else console.log("WARNING: entity_save - unsupported type: "+a)}catch(f){console.log("entity_save - "+f)}}function entity_types(){return"comment file node taxonomy_term taxonomy_vocabulary user".split(" ")}
...
function file_load(a,b){try{entity_load("file",a,b)}catch(c){console.log("file_load - "+c)}}function file_save(a,b){try{entity_save("file",null,a,b)}catch(c){console.log("file_save - "+c)}}function node_load(a,b){try{entity_load("node",a,b)}catch(c){console.log("node_load - "+c)}}function node_save(a,b){try{entity_save("node",a.type,a,b)}catch(c){console.log("node_save - "+c)}}function taxonomy_term_load(a,b){try{entity_load("taxonomy_term",a,b)}catch(c){console.log("taxonomy_term_load - "+c)}}
...
function entity_update(a,b,c,d){try{var e=_entity_wrap(a,c),f=entity_primary_key(a),g=null,g="comment"==a?entity_assemble_data(a,b,c,d):JSON.stringify(e);Drupal.services.call({method:"PUT",path:a+"/"+c[f]+".json",service:d.service,resource:d.resource,entity_type:a,entity_id:c[entity_primary_key(a)],bundle:b,data:g,success:function(b){try{_entity_local_storage_delete(a,c[f]),d.success&&d.success(b)}catch(e){console.log("entity_update - success - "+e)}},error:function(a,b,c){try{d.error&&d.error(a,
b,c)}catch(e){console.log("entity_update - error - "+e)}}})}catch(h){console.log("entity_update - "+h)}}
...
(f=encodeURIComponent(h),g=encodeURIComponent(a.parameters_op[h]),e+="parameters_op["+f+"]="+g+"&");""!=e&&(e=e.substring(0,e.length-1),b+=e+"&")}"undefined"!==typeof a.page&&(b+="page="+encodeURIComponent(a.page)+"&");"undefined"!==typeof a.page_size&&(b+="page_size="+encodeURIComponent(a.page_size)+"&");return b.substring(0,b.length-1)}catch(k){console.log("entity_index_build_query_string - "+k)}}
