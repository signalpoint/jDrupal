...
Drupal.services.call=function(a){try{if(a.debug=!1,services_ready())if(a.hasOwnProperty("tries")?a.tries++:(module_invoke_all("services_preprocess",a),a.tries=1),drupalgap.online){var b=new XMLHttpRequest,c=Drupal.settings.site_path+Drupal.settings.base_path+"?q=";"undefined"===typeof a.endpoint?c+=Drupal.settings.endpoint+"/":""!=a.endpoint&&(c+=a.endpoint+"/");var c=c+a.path,d=a.method.toUpperCase();Drupal.settings.debug&&console.log(d+": "+c);b.onload=function(e){try{if(4==b.readyState){var g=
b.status+" - "+http_status_code_title(b.status);if(200==b.status)Drupal.settings.debug&&console.log("200 - OK"),e=null,e=-1!=b.getResponseHeader("Content-Type").indexOf("application/json")?JSON.parse(b.responseText):b.responseText,module_invoke_all("services_request_pre_postprocess_alter",a,e),a.success(e,a.tries),module_invoke_all("services_request_postprocess_alter",a,e),module_invoke_all("services_postprocess",a,e),_services_process_submission_queue();else{Drupal.settings.debug&&(console.log(d+
": "+c+" - "+g),console.log(b.responseText),console.log(b.getAllResponseHeaders()));b.responseText?console.log(b.responseText):dpm(b);if("undefined"!==typeof a.error){var h=b.responseText||"";h&&""!=h||(h=g);1==a.tries&&a.error(b,b.status,h,a.tries)}module_invoke_all("services_postprocess",a,b)}}else console.log("Drupal.services.call - request.readyState = "+b.readyState)}catch(k){Drupal.settings.debug&&(console.log(d+" (ERROR): "+c+" - "+g),console.log(b.responseText),console.log(b.getAllResponseHeaders())),
console.log("Drupal.services.call - onload - "+k)}};services_get_csrf_token({debug:a.debug,success:function(e){try{var g=!0;"undefined"!==typeof a.async&&!1===a.async&&(g=!1);b.open(d,c,g);g=null;"POST"==d?(g="application/json","user"==a.service&&"login"==a.resource&&(g="application/x-www-form-urlencoded")):"PUT"==d&&(g="application/json");a.contentType&&(g=a.contentType);g&&b.setRequestHeader("Content-type",g);e&&b.setRequestHeader("X-CSRF-Token",e);"undefined"!==typeof a.data?(Drupal.settings.debug&&
(e=!0,"user"==a.service&&in_array(a.resource,["login","create","update"])&&(e=!1),e&&("object"===typeof a.data?console.log(JSON.stringify(a.data)):console.log(a.data))),b.send(a.data)):b.send(null)}catch(h){console.log("Drupal.services.call - services_get_csrf_token - success - "+h)}},error:function(b,c,d){try{console.log("Drupal.services.call - services_get_csrf_token - "+d),a.error&&a.error(b,c,d)}catch(e){console.log("Drupal.services.call - services_get_csrf_token - error - "+e)}}})}else module_invoke_all("services_postprocess",
a,0),a.hasOwnProperty("retry")&&a.retry&&_services_queue_submission(a),"undefined"!==typeof a.queued&&a.queued(a);else a.error(null,null,"Set the site_path and endpoint on Drupal.settings!")}catch(e){console.log("Drupal.services.call - error - "+e)}};
...
function _services_queue_callback_add(a,b,c,d,e){try{Drupal.services_queue[a][b][c][d].push(e)}catch(f){console.log("_services_queue_callback_add - "+f)}}function _services_queue_callback_count(a,b,c,d){try{return Drupal.services_queue[a][b][c][d].length}catch(e){console.log("_services_queue_callback_count - "+e)}}
function _services_process_submission_queue(){try{var a=_services_dequeue_submission();"undefined"!==typeof a&&Drupal.services.call(a)}catch(b){console.log("_services_process_submission_queue - "+b)}}function _services_get_submission_queue(){try{var a=variable_get("services_submission_queue",[]);"string"===typeof a&&(a=JSON.parse(a));return a}catch(b){console.log("_services_get_submission_queue - "+b)}}
function _services_set_submission_queue(a){try{variable_set("services_submission_queue",a)}catch(b){console.log("_services_set_submission_queue - "+b)}}function _services_queue_submission(a){try{var b=_services_get_submission_queue();b.push(a);_services_set_submission_queue(b)}catch(c){console.log("_services_queue_submission - "+c)}}
function _services_dequeue_submission(){try{var a=_services_get_submission_queue(),b=a.shift();_services_set_submission_queue(a);return b}catch(c){console.log("_services_dequeue_submission - "+c)}}function comment_create(a,b){try{services_resource_defaults(b,"comment","create"),entity_create("comment",null,a,b)}catch(c){console.log("comment_create - "+c)}}
