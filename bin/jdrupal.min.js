var Drupal=Drupal||drupal_init();Drupal.sessid=null;Drupal.csrf_token=!1;Drupal.user=drupal_user_defaults();function dpm(a){try{a&&("object"===typeof a?console.log(JSON.stringify(a)):console.log(a))}catch(b){console.log("dpm - "+b)}}function drupal_init(){return{settings:{base_path:"/",cache:{entity:{enabled:!1,expiration:3600}},endpoint:"",file_public_path:"sites/default/files",language_default:"und",site_path:""}}}function drupal_user_defaults(){return{uid:"0",roles:{1:"anonymous user"}}}
...
function comment_load(a,b){try{entity_load("comment",a,b)}catch(c){console.log("comment_load - "+c)}}function comment_save(a,b){try{entity_save("comment",null,a,b)}catch(c){console.log("comment_save - "+c)}}
...
c.responseText,Drupal.sessid=b,a.success&&a.success(b))}else console.log("services_get_csrf_token - readyState - "+c.readyState)}catch(h){console.log("services_get_csrf_token - token_request. onload - "+h)}};c.open("GET",d,!0);c.send(null)}}catch(e){console.log("services_get_csrf_token - "+e)}}
...
"&");"undefined"!==typeof a.page_size&&(b+="page_size="+encodeURIComponent(a.page_size)+"&");return b.substring(0,b.length-1)}catch(h){console.log("entity_index_build_query_string - "+h)}}function _entity_wrap(a,b){try{var c={};"taxonomy_term"==a||"taxonomy_vocabulary"==a||"user"==a?c=b:c[a]=b;return c}catch(d){console.log("_entity_wrap - "+d)}}function node_create(a,b){try{entity_create("node",a.type,a,b)}catch(c){console.log("node_create - "+c)}}
