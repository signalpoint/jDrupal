...
Drupal.services.call=function(a){try{if(a.debug=!1,services_ready()){var b=new XMLHttpRequest,c=Drupal.settings.site_path+Drupal.settings.base_path+"?q=";"undefined"===typeof a.endpoint?c+=Drupal.settings.endpoint+"/":""!=a.endpoint&&(c+=a.endpoint+"/");var c=c+a.path,d=a.method.toUpperCase();Drupal.settings.debug&&console.log(d+": "+c);b.onload=function(e){try{if(4==b.readyState){var g=b.status+" - "+http_status_code_title(b.status);if(200==b.status){Drupal.settings.debug&&console.log("200 - OK");
var h=JSON.parse(b.responseText);module_invoke_all("services_request_pre_postprocess_alter",a,h);a.success(h);module_invoke_all("services_request_postprocess_alter",a,h)}else if(Drupal.settings.debug&&(console.log(d+": "+c+" - "+g),dpm(b)),b.responseText?console.log(b.responseText):dpm(b),"undefined"!==typeof a.error){var k=b.responseText||"";k&&""!=k||(k=g);a.error(b,b.status,k)}}else console.log("Drupal.services.call - request.readyState = "+b.readyState)}catch(l){console.log("Drupal.services.call - onload - "+
l)}};services_get_csrf_token({debug:a.debug,success:function(e){try{b.open(d,c,!0),"POST"==d?b.setRequestHeader("Content-type","application/x-www-form-urlencoded"):"PUT"==d&&b.setRequestHeader("Content-type","application/json"),e&&b.setRequestHeader("X-CSRF-Token",e),"undefined"!==typeof a.data?(Drupal.settings.debug&&("user"!=a.service||in_array(a.resource,["login","create","update"])||("object"===typeof a.data?console.log(JSON.stringify(a.data)):console.log(a.data))),b.send(a.data)):b.send(null)}catch(g){console.log("Drupal.services.call - services_get_csrf_token - success - "+
