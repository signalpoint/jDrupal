...
console.log("200 - OK");var h=JSON.parse(b.responseText);module_invoke_all("services_request_pre_postprocess_alter",a,h);a.success(h);module_invoke_all("services_request_postprocess_alter",a,h);module_invoke_all("services_postprocess",a,h)}else{Drupal.settings.debug&&(console.log(d+": "+c+" - "+g),console.log(b.getAllResponseHeaders()));b.responseText?console.log(b.responseText):dpm(b);if("undefined"!==typeof a.error){var k=b.responseText||"";k&&""!=k||(k=g);a.error(b,b.status,k)}module_invoke_all("services_postprocess",
a,b)}}else console.log("Drupal.services.call - request.readyState = "+b.readyState)}catch(l){console.log("Drupal.services.call - onload - "+l)}};services_get_csrf_token({debug:a.debug,success:function(e){try{var g=!0;"undefined"!==typeof a.async&&!1===a.async&&(g=!1);b.open(d,c,g);"POST"==d?(g="application/json","file"==a.service?(g="application/json",a.data=JSON.stringify(a.data)):"user"==a.service&&"login"==a.resource&&(g="application/x-www-form-urlencoded"),b.setRequestHeader("Content-type",g)):
...
d),a.error&&a.error(b,c,d)}catch(e){console.log("Drupal.services.call - services_get_csrf_token - error - "+e)}}})}else a.error(null,null,"Set the site_path and endpoint on Drupal.settings!")}catch(e){console.log("Drupal.services.call - error - "+e)}};
