...
Drupal.services.call=function(a){try{if(a.debug=!1,services_ready()){a.hasOwnProperty("tries")?a.tries++:(module_invoke_all("services_preprocess",a),a.tries=1);var b=new XMLHttpRequest;b.timeout=Drupal.settings.timeout?Drupal.settings.timeout:5E3;var c=Drupal.settings.site_path+Drupal.settings.base_path+"?q=";"undefined"===typeof a.endpoint?c+=Drupal.settings.endpoint+"/":""!=a.endpoint&&(c+=a.endpoint+"/");var c=c+a.path,d=a.method.toUpperCase();Drupal.settings.debug&&console.log(d+": "+c);b.addEventListener("error",
function(b){_services_submission_error_handler(a);drupalgap_alert("SERVICE ERROR")});b.addEventListener("abort",function(b){_services_submission_error_handler(a);drupalgap_alert("SERVICE ABORT")});b.addEventListener("timeout",function(b){_services_submission_error_handler(a);drupalgap_alert("SERVICE TIMEOUT")});b.onload=function(e){try{var g=b.status+" - "+http_status_code_title(b.status);if(200==b.status)Drupal.settings.debug&&console.log("200 - OK"),e=null,e=-1!=b.getResponseHeader("Content-Type").indexOf("application/json")?
JSON.parse(b.responseText):b.responseText,module_invoke_all("services_request_pre_postprocess_alter",a,e),"undefined"!==typeof a.success&&a.success(e),module_invoke_all("services_request_postprocess_alter",a,e),module_invoke_all("services_postprocess",a,e);else{Drupal.settings.debug&&(console.log(d+": "+c+" - "+g),console.log(b.responseText),console.log(b.getAllResponseHeaders()));b.responseText?console.log(b.responseText):dpm(b);if("undefined"!==typeof a.error){var h=b.responseText||"";h&&""!=h||
(h=g);a.error(b,b.status,h)}a.retry=!1;_services_submission_error_handler(a)}_services_process_submission_queue()}catch(k){Drupal.settings.debug&&(console.log(d+" (ERROR): "+c+" - "+g),console.log(b.responseText),console.log(b.getAllResponseHeaders())),console.log("Drupal.services.call - onload - "+k),_services_submission_error_handler(a)}};services_get_csrf_token({debug:a.debug,success:function(e){try{var g=!0;"undefined"!==typeof a.async&&!1===a.async&&(g=!1);b.open(d,c,g);g=null;"POST"==d?(g="application/json",
"user"==a.service&&"login"==a.resource&&(g="application/x-www-form-urlencoded")):"PUT"==d&&(g="application/json");a.contentType&&(g=a.contentType);g&&b.setRequestHeader("Content-type",g);e&&b.setRequestHeader("X-CSRF-Token",e);"undefined"!==typeof a.data?(Drupal.settings.debug&&(e=!0,"user"==a.service&&in_array(a.resource,["login","create","update"])&&(e=!1),e&&("object"===typeof a.data?console.log(JSON.stringify(a.data)):console.log(a.data))),b.send(a.data)):b.send(null)}catch(h){console.log("Drupal.services.call - services_get_csrf_token - success - "+
h),_services_submission_error_handler(a)}},error:function(b,c,d){try{console.log("Drupal.services.call - services_get_csrf_token - "+d)}catch(e){console.log("Drupal.services.call - services_get_csrf_token - error - "+e)}_services_submission_error_handler(a)}})}else a.error(null,null,"Set the site_path and endpoint on Drupal.settings!")}catch(e){console.log("Drupal.services.call - error - "+e)}};
function services_get_csrf_token(a){try{var b;a.reset&&(Drupal.sessid=null);Drupal.sessid&&(b=Drupal.sessid);if(b)a.success&&a.success(b);else{var c=new XMLHttpRequest,d=Drupal.settings.site_path+Drupal.settings.base_path+"?q=services/session/token";c.addEventListener("error",function(b){a.error&&a.error();drupalgap_alert("CSRF TOKEN ERROR")});c.addEventListener("abort",function(b){a.error&&a.error();drupalgap_alert("CSRF TOKEN ABORT")});c.addEventListener("timeout",function(b){a.error&&a.error();
drupalgap_alert("CSRF TOKEN TIMEOUT")});c.onload=function(e){try{var g=c.status+" - "+http_status_code_title(c.status);200!=c.status?(a.error&&a.error(),console.log(d+" - "+g),console.log(c.responseText)):(b=c.responseText,Drupal.sessid=b,a.success&&a.success(b))}catch(h){a.error&&a.error(),console.log("services_get_csrf_token - token_request. onload - "+h)}};c.open("GET",d,!0);c.send(null)}}catch(e){console.log("services_get_csrf_token - "+e)}}
...
function _services_queue_callback_count(a,b,c,d){try{return Drupal.services_queue[a][b][c][d].length}catch(e){console.log("_services_queue_callback_count - "+e)}}function _services_process_submission_queue(){try{var a=_services_dequeue_submission();"undefined"!==typeof a&&Drupal.services.call(a)}catch(b){console.log("_services_process_submission_queue - "+b)}}
function _services_get_submission_queue(){try{var a=variable_get("services_submission_queue",[]);"string"===typeof a&&(a=JSON.parse(a));return a}catch(b){console.log("_services_get_submission_queue - "+b)}}function _services_set_submission_queue(a){try{variable_set("services_submission_queue",a)}catch(b){console.log("_services_set_submission_queue - "+b)}}
function _services_queue_submission(a){try{var b=_services_get_submission_queue();b.push(a);_services_set_submission_queue(b)}catch(c){console.log("_services_queue_submission - "+c)}}function _services_dequeue_submission(){try{var a=_services_get_submission_queue(),b=a.shift();_services_set_submission_queue(a);return b}catch(c){console.log("_services_dequeue_submission - "+c)}}
function _services_submission_error_handler(a){try{module_invoke_all("services_postprocess",a,0),a.hasOwnProperty("retry")&&a.retry?(_services_queue_submission(a),"undefined"!==typeof a.queued&&a.queued(a)):"undefined"!==typeof a.error&&a.error(xhr,status,message)}catch(b){console.log("_services_submission_error_handler - "+b)}}function comment_create(a,b){try{services_resource_defaults(b,"comment","create"),entity_create("comment",null,a,b)}catch(c){console.log("comment_create - "+c)}}
...
a.options[l];for(k in m)m.hasOwnProperty(k)&&(f=encodeURIComponent(k),g=encodeURIComponent(m[k]),h+="options["+l+"]["+f+"]="+g+"&")}""!=h&&(h=h.substring(0,h.length-1),b+=h+"&")}"undefined"!==typeof a.page&&(b+="page="+encodeURIComponent(a.page)+"&");"undefined"!==typeof a.page_size&&(b+="pagesize="+encodeURIComponent(a.page_size)+"&");return b.substring(0,b.length-1)}catch(n){console.log("entity_index_build_query_string - "+n)}}
function _entity_wrap(a,b){try{var c={};"comment"==a||"taxonomy_term"==a||"taxonomy_vocabulary"==a||"user"==a||-1!=a.indexOf("commerce")?c=b:c[a]=b;return c}catch(d){console.log("_entity_wrap - "+d)}}function file_create(a,b){try{services_resource_defaults(b,"file","create"),entity_create("file",null,a,b)}catch(c){console.log("file_create - "+c)}}
